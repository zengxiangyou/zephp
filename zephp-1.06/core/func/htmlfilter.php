...
class z_core_func_htmlfilter{
    ...
    function htmlFilter ($str) {
        if (!is_array($str)) {
            ...
            $str = preg_replace("/cookie/si", "coo-kie", $str); //cookie
            $str = preg_replace("/javascript/si", "java-script", $str); //javascript
            $str = preg_replace("/vbscript/si", "vb-script", $str); //vbscript
            $str = preg_replace("/on([a-z]+)\s*=/si", "on-\\1=", $str); //js事件
