...
function z1_common_tableImport($f, $dbname = ''){
	$pass = 1;
	if ($dbname != '') {
		$row = z_db_rows("SELECT * FROM information_schema.SCHEMATA where SCHEMA_NAME='{$dbname}'");
		if (empty($row)) {
		    zz::incPhp('core-z-mysql');
			$r = z_mysql_databaseCreate($dbname);
			if (!$r) $pass = 0;
		}
	...
	if ($pass) {
		...
		if (!empty($sql)) {
			...
			if (preg_match_all('/(SET|CREATE|INSERT|ALTER)(.*?);(\s*\n|\s*$)/is', $sql, $m)) {
				foreach ($m[1] as $k => $r) {
					$r = $r . $m[2][$k];
					if (!z_db_query($r)) {
